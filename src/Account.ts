...
export class Account implements Account {
    ...
    private updateBalance(): void {
        const balance: number = this.entries.reduce((prevValue, currentEntry) => prevValue + this.convertAmountByCategory(currentEntry), 0);
    ...
    private convertAmountByCategory(entry: Entry): number {
        ...
        if(category === CategoryEnum.expense) return -amount;
